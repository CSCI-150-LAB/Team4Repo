...
<script type="text/javascript">
	...
	function changeDisplayCount($displayCount) {
		...
		$startList = 0; // this should update on page change (next and prev clicks)
		$fetchEntries = "SELECT * FROM listingbase WHERE listing_itemtype='Other' AND listing_id >= $startList AND listing_id < ($startList + $listEntries)";
	...
	function callEntries($change) {
		...
		$fetchEntries = "SELECT * FROM listingbase WHERE listing_itemtype='Others' AND listing_id >= $startList AND listing_id < ($startList + $listEntries)";
		$entries = $conn->query($fetchEntries);
