...
<script type="text/javascript">
	...
	function changeDisplayCount($displayCount) {
		...
		$startList = 0; // this should update on page change (next and prev clicks)
		$fetchEntries = "SELECT * FROM post_base WHERE post_ID >= $startList AND post_ID < ($startList + $listEntries)";
	...
	function callEntries($change) {
		...
		if ($itemselection == 'null'){
            ...
            $fetchEntries = "SELECT * FROM post_base WHERE post_class = $itemselection AND post_ID >= $startList AND post_ID < ($startList + $listEntries)";
	...
	function initialListings() {
        ...
        for (var i = 0; i < jsArr.length; i++) {
            ...
            var pageLink = "exListing1.php?postID=" + jsArr[i][0]; // this is temp until directories are stored in db
